...
The attached archive contains every `*.h` and `*.hpp` file produced by an iOS `template_debug` SCons build of that Godot version. These headers are useful for native iOS plugins that need to compile against Godot's internal API without building the engine from source themselves.
